...
const CreateProfile = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    gender: "",
    role: "",
    isActive: "pending", // Default to pending until approved
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        ...
        toast.info(`Welcome ${updatedFormData.name.split(' ')[0]}. Your account is pending approval.`);

        // Auto-logout since account is pending
        if (logout()) {
          navigate("/login");
        }
      }
  ...
  return (
    ...
    >
      <button onClick={() => navigate(-1)} className="mx-10 mt-5">
      ...
      <UserForm
        ...
        authorized={false} // This is self-registration
